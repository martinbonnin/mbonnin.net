...
When it comes to Kotlin and Gradle everything is "Kotlin DSL". But what "DSL" means exactly? I'm unclear.
...
Similarly, you would think DSL has some declarative aspect to it but it's mostly programmatic under the hood. The order of your blocks may be important.
...
Of course a declarative approach would be nice and allow us to clearly separate concerns. I hope [Declarative Gradle](https://blog.gradle.org/declarative-gradle) will fix this situation. But for I like to think **Kotlin build scripts** are well defined. **Kotlin DSL** is a bit in between.
...
If you copy/paste your code to someone who doesn't have those plugins enabled, it's just going to fail:
...
This is because `publication` signature is `void publications(Action<PublicationContainer> configure)`. The `sam-with-receiver` plugin turns the `Action<>` type parameter into a receiver. But without the plugin, you need to reference the parameter:
...
This wouldn't be an issue if the dialect was required to write Gradle plugins. I come from [Occitania](https://en.wikipedia.org/wiki/Occitania) and enjoy a good regional dialect. After all, everyone shares Jetpack Compose code happily. `@Composable` is vocabulary you need to have to talk Compose Kotlin. But you also can't really do Jetpack Compose without the Compose compiler plugin.
...
`kotlin-dsl` is slower due to "precompiled script plugins". Generating the accessor, etc.. adds up.
...
This is not needed. [You can use newer tools and have them target older version](https://blog.alllex.me/posts/2023-11-03-liberal-library-tooling/).
...
Overall, `kotlin-dsl` is making a lot of decisions for you. Some of them you might be comfortable with (Do you need the `java-gradle-plugin` applied? Probably?) some maybe not (Are the Kotlin plugin compilers really mandatory?) while some of them are not so clear (Do I need `-java-parameters`? Maybe?).
